...
export default function Footer() {
  ...
  return (
    <footer className="px-6 lg:px-16 py-10 flex flex-wrap items-center justify-between gap-5" style={{ background: "#1A0F08" }}>
      <div>
      ...
      <ul className="hidden md:flex gap-6 list-none">
        {navItems.map(l => (
          <li key={l.label}>
            <Link href={l.href} className="text-xs tracking-wide" style={{ color: "rgba(255,255,255,0.35)", textDecoration: "none" }}
              onMouseEnter={e => (e.currentTarget.style.color = "#F5A65B")}
              onMouseLeave={e => (e.currentTarget.style.color = "rgba(255,255,255,0.35)")}>{l.label}</Link>
          </li>
        ))}
      </ul>
