    ...
    <section id="contact" className="py-24 px-6 lg:px-16 text-center" style={{ background: "#2C1810" }} ref={ref}>
      <div className="max-w-2xl mx-auto">
        <div className="reveal">
          ...
          <h2 className="font-playfair font-bold leading-tight mb-5" style={{ fontSize: "clamp(32px,4vw,52px)", color: "white" }}>
            {c.h2a}<br />{c.h2b} <em className="font-playfair" style={{ color: "#F5A65B", fontStyle: "italic" }}>{c.h2c}</em>
          </h2>
          <p className="font-cormorant font-light leading-relaxed mb-12" style={{ fontSize: 20, color: "rgba(255,255,255,0.5)" }}>{c.sub}</p>
        </div>
        ...
        {!submitted ? (
          <form className="reveal flex flex-col gap-4 text-left" onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <input type="text" placeholder={c.surname} style={inputStyle} />
              <input type="text" placeholder={c.city} style={inputStyle} />
            </div>
            <select style={{ ...inputStyle, cursor: "pointer" }} defaultValue="">
              <option value="" disabled>{c.packageLabel}</option>
              {c.packageOptions.map((o) => <option key={o} value={o}>{o}</option>)}
            </select>
            <textarea placeholder={c.storyPlaceholder} rows={5} style={{ ...inputStyle, resize: "vertical" }} />
            <button type="submit" className="rounded-full font-medium transition-all mt-2"
              style={{ background: "#E8721A", color: "white", padding: "16px", fontSize: 14, border: "none", cursor: "pointer" }}
          ...
          <div className="reveal py-12">
            <p className="font-cormorant italic" style={{ fontSize: 24, color: "rgba(255,255,255,0.7)" }}>{c.thanks}</p>
          </div>
        )}
      </div>
    </section>
